...
const ShoppingCart: React.FC<ShoppingCartProps> = ({ cartItems, setCartItems }) => {
  ...
  const handleQuantityChange = (item: ItemType, quantity: number) => {
    ...
    const existingItemIndex = updatedCartItems.findIndex((cartItem) => cartItem.item.item_id === item.item_id);
  ...
  const calculateTotal = () => {
    return cartItems.reduce((total, cartItem) => total + cartItem.item.item_price * cartItem.quantity, 0);
  };
      ...
      {isCartOpen && (
        <div className="fixed inset-y-0 left-0 bg-white shadow-lg w-80 z-50 transform transition-transform duration-300 ease-in-out translate-x-0">
          <div className="p-6 relative">
            <button onClick={toggleCart} className="absolute top-2 right-2 text-gray-600 hover:text-gray-900">
              <FaTimes size={20} />
            </button>
              ...
              <ul>
                {cartItems.map(({ item, quantity }) => (
                  <li key={item.item_id} className="flex justify-between items-center mb-2">
                    <span className="flex-1 mr-2 overflow-hidden text-ellipsis whitespace-nowrap" title={item.item_name}>
                      {item.item_name}
                    </span>
                    <div className="flex items-center" style={{ width: "80px" }}>
                      ...
                      />
                      <button onClick={() => handleQuantityChange(item, 0)} className="ml-2 text-red-600 hover:text-red-800">
                        <FaTimes />
                      </button>
                    </div>
                    <span className="w-16 text-right">${(item.item_price * quantity).toFixed(2)}</span>
                  </li>
                ))}
              </ul>
            ) : (
            ...
            <div className="flex justify-between items-center mt-2 font-bold">
              <button onClick={proceedToCheckout} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                Pay Now
              </button>
              <span className="mb-6">Total: ${calculateTotal().toFixed(2)}</span>
            </div>
          </div>
        </div>
      )}
